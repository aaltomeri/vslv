...
var PAGE_MODULE = function() {
    var currentPage, module = _.extend({}, Backbone.Events), name = "page", Model = Backbone.Model.extend({
        initialize: function() {},
        getMediasInfos: function() {
            var attachments = this.get("post_meta").attachments || null, medias = [], requests_promises = [];
            if (attachments) return attachments = $.parseJSON(attachments[0]), _.each(attachments.attachments, function(attachment) {
                var promise = $.get(VSLV_CONFIG.base_url + "media/" + attachment.id).success(function(data) {
                    medias.push(data);
                }).error(function() {
                ...
                this.set("medias", medias), this.trigger("Page:MediasLoaded");
            }, this)), requests_promises;
        }
    }), Collection = Backbone.Collection.extend({
        url: VSLV_CONFIG.base_url + VSLV_CONFIG.modules[name].route + "?lang=" + VSLV_CONFIG.lang,
        model: Model
    }), init = (Backbone.View.extend({}), function(currentPost_data) {
        return currentPage = new Model(currentPost_data);
    });
    return module.collection = new Collection(), module.init = init, module;
    ...
    var portfolioView, module = _.extend({}, Backbone.Events), name = "project", Model = Backbone.Model.extend({
        mediasLoaded: !1,
        initialize: function() {
        ...
        getMediasInfos: function() {
            ...
            attachments = $.parseJSON(attachments[0]), _.each(attachments.projects_medias, function(attachment) {
            ...
            return $.when.call($, promise).done($.proxy(function() {
                this.set("medias", medias), this.trigger("Project:MediasLoaded");
            }, this)), promise;
        }
    }), Collection = Backbone.Collection.extend({
        url: VSLV_CONFIG.base_url + VSLV_CONFIG.modules[name].route + "?lang=" + VSLV_CONFIG.lang,
        ...
        initialize: function() {
            ...
            this.on("reset", function() {
                module.trigger("Projects:loaded"), this.each(function() {}), $.when.apply($, medias_promises).done($.proxy(function() {
                    module.trigger("Projects:loaded");
                }, this));
            });
        }
    }), collection = new Collection(), PortofolioView = Backbone.View.extend({
        id: "portfolio",
        items: [],
        initialize: function() {
        ...
        preloadThumbs: function() {
            ...
            }), project_thumbs_queue.on("complete", this.render, this), project_thumbs_queue.loadManifest(project_thumbs_loading_manifest);
        },
        render: function() {
            this.collection.each(function(model) {
                ...
                item.render(), this.$el.append(item.$el);
            }, this);
        }
        ...
        initialize: function() {},
        render: function() {
            var $link = $("<a />").attr("href", this.model.get("link")), $image = $("<img />").attr("src", this.model.get("thumbnail").url);
            return this.$el.html($link.append($image)), this;
        },
        launchProject: function() {}
    }), init = function(projects) {
        portfolioView = new PortofolioView({
            el: "#portfolio",
            collection: collection
        }), projects.length ? collection.reset(projects) : collection.fetch({
            reset: !0,
            data: {
                filter: {
                    orderby: "title",
    ...
    return module.collection = collection, module.init = init, module;
}(window, jQuery, createjs), VSLV_APP = function(page_module, project_module, app_data) {
    {
        var currentPage, discoveries, Router = Backbone.Router.extend({
            routes: {
                ":slug": "page",
    ...
    return {
        ...
        common: {
            init: function() {
                app_data = this.parse_app_data(), Backbone.history.start({
                    pushState: !0,
                    root: VSLV_APP.root
                }), currentPage = page_module.init(app_data.currentPost), project_module.on("Projects:loaded", function() {
                    ...
                    }), discoveries = new DiscoveryCollection(_discoveries), currentPage.on("Page:MediasLoaded", function() {
                        discoveries.findWhere({
                            title: currentPage.get("title")
                        }).set("medias", this.get("medias"));
                    }, currentPage), currentPage.getMediasInfos();
                }), project_module.init(app_data.projects);
            },
            parse_app_data: function() {
                return _.each(app_data, function(value, key) {
                    try {
