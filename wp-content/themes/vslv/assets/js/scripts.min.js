...
var DISCOVERY_MODULE = function() {
    var module = _.extend({
        ...
        setPosition: function() {
            var $parent = this.$el.parent(), pw = $parent.width(), ph = $parent.height(), w = this.$el.width(), h = this.$el.height(), x_min = $parent.scrollLeft() + w, x_max = pw + $parent.scrollLeft() - w * 2, x_range = x_max - x_min, y_min = $parent.scrollTop() + $("header").outerHeight() + h * 4, y_max = ph + $parent.scrollTop() - $("header").outerHeight() - h * 4, y_range = y_max - y_min, x = Math.min(Math.max(Math.random() * x_range, x_min), x_max), y = Math.min(Math.max(Math.random() * y_range, y_min), y_max);
            this.$el.css({
                top: Math.round(y),
                left: Math.round(x)
            });
