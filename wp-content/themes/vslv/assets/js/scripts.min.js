var VSLV_CONFIG = {
    ...
    discovery_hide_content_delay: 5e3,
    discovery_hint_interval: 14e3,
    discovery_hint_show_duration: 2e3,
    discovery_hint_hide_duration: 500,
    first_media_layer_opacity: .7,
    modules: {
        page: {
            name: "page",
            route: "pages"
    ...
    $.fn.transitionStop = $.fn.transitStop = function(clearQueue, jumpToEnd) {
        this.each(function() {
            ...
            if (properties) {
                ...
                var style = window.getComputedStyle(this), css = {};
    ...
    _this.startAutoplay = function() {
        if (_this.support.transitions) {
            ...
            autoplayIntervalId = setInterval(function() {
                if (params.loop) _this.swipeNext(true); else if (!_this.swipeNext(true)) {
...
var DISCOVERY_MODULE = function() {
    var module = _.extend({
        ...
        setCurrentMedia: function(index) {
            ...
            if (cm) {
                var type = this.get("type"), slug = this.get("slug"), route = VSLV_CONFIG.modules[type].route, url = route + "/" + slug + "/" + (this.currentMediaIndex + 1) + window.location.search;
                if (type === "project") {
        ...
        getMediaSource: function(mediaObject) {
            var size = (window.innerWidth < VSLV_CONFIG.discovery_wp_image_small_size_breakpoint ? VSLV_CONFIG.discovery_wp_image_small_size : VSLV_CONFIG.discovery_wp_image_size) || null, thumbnail_o = size ? mediaObject.attachment_meta.sizes[size] : undefined, source = typeof thumbnail_o !== "undefined" ? thumbnail_o.url : mediaObject.source;
            return source;
        }
        ...
        onClickHandler: function(e) {
            ...
            var view = this, pointer_x = e.clientX || e.originalEvent.touches[0].clientX || e.originalEvent.pageX, pointer_y = e.clientY || e.originalEvent.touches[0].clientY || e.originalEvent.pageY;
        ...
        onMousedownHandler: function(e) {
            var view = this, pointer_x = e.clientX || e.originalEvent.touches[0].clientX || e.originalEvent.pageX, pointer_y = e.clientY || e.originalEvent.touches[0].clientY || e.originalEvent.pageY, pointer_radius = this.pointer_radius, c = this.c;
            pointer_x += view.el.scrollLeft;
            pointer_y += view.el.scrollTop;
            this.pointer_x = pointer_x;
            this.pointer_y = pointer_y;
            var mediaObject = this.model.getNextMedia(), mediaElement;
            if (this.loadQueue !== null && this.loadQueue.getResult(mediaObject.slug) instanceof HTMLImageElement) {
                ...
                this.$el.on("mousemove touchmove", function(e) {
                    var pointer_x = e.clientX || e.originalEvent.touches[0].clientX || e.originalEvent.pageX, pointer_y = e.clientY || e.originalEvent.touches[0].clientY || e.originalEvent.pageY;
                    e.preventDefault();
                    pointer_x += view.el.scrollLeft;
                    pointer_y += view.el.scrollTop;
        ...
        renderImage: function(mediaObject) {
            ...
            if (this.loadQueue !== null && this.loadQueue.getResult(mediaObject.slug) instanceof HTMLImageElement) {
                ...
                }, this.windowResizeHandlerForImage);
            } else {
        ...
        windowResizeHandlerForImage: function(e) {
            var view = e.data.view, mediaElement = e.data.mediaElement;
            view.drawMediaOnCanvas(mediaElement, view.c);
            console.log("resize 2");
        },
        renderVideo: function(mediaObject) {
            console.log("video: ", mediaObject.source);
            var view = this, mediaElement = $("<video muted></video>").get(0), sources_html, source_to_draw_on_canvas;
        ...
        initVideoControls: function() {
            var view = this, $video = this.$("video"), video = $video.get(0), $bwd = this.$(".control-bwd"), $fwd = this.$(".control-fwd"), $play = this.$(".control-play"), $sound = this.$(".control-sound"), $controls = this.$(".controls");
            var play_src = $play.find("img").get(0).src, play_new_src = play_src.replace("-pause", "-play");
            $play.find("img").get(0).src = play_new_src;
            $video.on("playing", function() {
                var src = $play.find("img").get(0).src, new_src = src.replace("-play", "-pause");
            ...
            $video.on("pause", function() {
                var src = $play.find("img").get(0).src, new_src = src.replace("-pause", "-play");
                $play.find("img").get(0).src = new_src;
                if (device.iphone()) {
                    $video.hide();
                }
            ...
            $video.on("volumechange", function() {
                var src = $sound.find("img").get(0).src, new_src;
                if (video.muted) {
        ...
        unbindVideoControlsEvents: function() {
            var $video = this.$("video"), video = $video.get(0), $bwd = this.$(".control-bwd"), $fwd = this.$(".control-fwd"), $play = this.$(".control-play"), $sound = this.$(".control-sound");
            $video.off();
            $bwd.off();
            $fwd.off();
            $play.off();
        ...
        setVideoDimensions: function(videoElement) {
            var master_dimension = this.c.width > this.c.height ? "width" : "height", other_dimension = master_dimension === "width" ? "height" : "width", md_value = this.c[master_dimension] / window.devicePixelRatio, od_value = "auto";
            if (device.ios()) {
                od_value = "100%";
        ...
        getMediaElementWithLayer: function(mediaElement, color, alpha) {
            var c1 = document.createElement("canvas"), ctx1 = c1.getContext("2d"), c2 = document.createElement("canvas"), ctx2 = c2.getContext("2d"), sw = mediaElement instanceof HTMLVideoElement ? mediaElement.videoWidth : mediaElement.width, sh = mediaElement instanceof HTMLVideoElement ? mediaElement.videoHeight : mediaElement.height, scale_h = this.$el.width() / sw, scale_v = this.$el.height() / sh, scale = scale_h > scale_v ? scale_h : scale_v, dw = sw * scale, dh = sh * scale;
        ...
        adaptCanvasToHiDPIScreen: function(canvasElement, bypass) {
            var c = canvasElement, ctx = c.getContext("2d"), devicePixelRatio = window.devicePixelRatio, backingStoreRatio = this.getBackingStoreRatio(ctx), ratio = this.getDevicePixelBackingStoreRatio(ctx), _bypass = bypass || false;
        ...
        getDevicePixelBackingStoreRatio: function(ctx) {
            var devicePixelRatio = window.devicePixelRatio || 1, backingStoreRatio = this.getBackingStoreRatio(ctx), ratio = devicePixelRatio / backingStoreRatio;
            return ratio;
        },
        replaceCanvasWithBgImage: function(mediaElement, canvasElement) {
            if (mediaElement instanceof HTMLImageElement) {
                this.$el.css({
                    "background-image": "url(" + mediaElement.src + ")"
                });
                $(canvasElement).transit({
                    opacity: 0
                });
            }
        },
        drawMediaOnCanvas: function(mediaElement, canvasElement) {
            var m = mediaElement, c = canvasElement, ctx = c.getContext("2d"), sw = mediaElement instanceof HTMLVideoElement ? mediaElement.videoWidth : mediaElement.width, sh = mediaElement instanceof HTMLVideoElement ? mediaElement.videoHeight : mediaElement.height, scale_h = this.$el.width() / sw, scale_v = this.$el.height() / sh, scale = scale_h > scale_v ? scale_h : scale_v, dw = Math.round(sw * scale), dh = Math.round(sh * scale);
            c.width = c.width;
            c.width = dw;
            c.height = dh;
        ...
        drawMediaOnCanvasAnimate: function(mediaElement, canvasElement) {
            var view = this, m = mediaElement, c = canvasElement, ctx = c.getContext("2d"), c2 = this.c2, ctx2 = this.ctx2, c3 = this.c3, ctx3 = this.ctx3, sw = mediaElement instanceof HTMLVideoElement ? mediaElement.videoWidth : mediaElement.width, sh = mediaElement instanceof HTMLVideoElement ? mediaElement.videoHeight : mediaElement.height, radius = view.pointer_radius, _step = 40, $this = $(this), scale_h = this.$el.width() / sw, scale_v = this.$el.height() / sh, scale = scale_h > scale_v ? scale_h : scale_v, dw = Math.round(sw * scale), dh = Math.round(sh * scale), pointer_x = this.pointer_x ? this.pointer_x : Math.random() * dw, pointer_y = this.pointer_y ? this.pointer_y : Math.random() * dh;
            var $tmp_c = $(c).clone(), tmp_c = $tmp_c.get(0), tmp_ctx = tmp_c.getContext("2d");
            tmp_c.width = c.width;
        ...
        gradient_draw: function(mediaElement, x, y, radius, canvasElement) {
            var view = this, m = mediaElement, c = canvasElement ? canvasElement : this.c, ctx = c.getContext("2d"), c2 = this.c2, ctx2 = this.ctx2, c3 = this.c3, ctx3 = this.ctx3, sw = mediaElement instanceof HTMLVideoElement ? mediaElement.videoWidth : mediaElement.width, sh = mediaElement instanceof HTMLVideoElement ? mediaElement.videoHeight : mediaElement.height, _grd_x = 0, _grd_y = 0, $this = $(this), scale_h = this.$el.width() / sw, scale_v = this.$el.height() / sh, scale = scale_h > scale_v ? scale_h : scale_v, dw = Math.round(sw * scale), dh = Math.round(sh * scale), dp_bs_ratio = 1;
            c3.width = c3.width;
        ...
        setPosition: function() {
            var $parent = this.$el.parent(), pw = $parent.width(), ph = $parent.height(), w = this.$el.width(), h = this.$el.height(), x_min = $parent.scrollLeft() + 20, x_max = pw + $parent.scrollLeft() - w, x_range = x_max - x_min, y_min = $parent.scrollTop() + $("header").outerHeight() + h, y_max = ph + $parent.scrollTop() - $("header").outerHeight() - h, y_range = y_max - y_min, x = Math.min(Math.max(Math.random() * x_range + x_min, x_min), x_max), y = Math.min(Math.max(Math.random() * y_range + y_min, y_min), y_max);
            this.$el.css({
                top: Math.round(y),
                left: Math.round(x)
            });
...
var PAGE_MODULE = function() {
    var module = _.extend({
        ...
        show: function() {
            var view = this, type = this.model.get("type"), title = this.model.get("title"), slug = this.model.get("slug"), content = this.model.get("content"), panel_type = this.model.get("panel-type");
        ...
        hide: function(offsetX) {
            var view = this, type = this.model.get("type"), slug = this.model.get("slug"), title = this.model.get("title"), content = this.model.get("content"), panel_type = this.model.get("panel-type"), animation_attributes = {};
        ...
        _scroll: function(e) {
            var el = e.currentTarget, direction = $(el).hasClass("arrow-up") ? 1 : -1, step = 100 * direction, current_pos = this.swiper.getWrapperTranslate("y");
    ...
    var module = _.extend({
        ...
        initialize: function() {
            if (this.get("featured_image")) {
                this.set("thumbnail", this.get("featured_image").attachment_meta.sizes["portfolio-thumbnail"]);
            }
            var panel_type = this.get("post_meta")["panel-type"] ? this.get("post_meta")["panel-type"][0] : "left";
            this.set("panel-type", panel_type);
        },
        ...
        onItemEnterHandler: function(e) {
            ...
            var $item = $(e.currentTarget), $img = $item.find("img:eq(0)"), $hoverElement = $("<div></div>").css({
                position: "absolute",
                top: -$img.height() / 2 + $item.height() / 2,
                right: 0
            }).addClass("portfolio-item-hover");
            if (!$item.find(".portfolio-item-hover").length) {
                $item.append($hoverElement);
            }
            var base_image_height = 250, base_font_size = $hoverElement.css("font-size").match(/^[0-9]+/)[0], base_line_height = $hoverElement.css("line-height").match(/^[0-9]+/)[0], font_size = base_font_size / base_image_height * $img.height(), line_height = base_line_height / base_image_height * $img.height();
            $hoverElement.css({
        ...
        onItemLeaveHandler: function(e) {
            ...
            var $item = $(e.currentTarget), $hoverElement = $item.find(".portfolio-item-hover");
            $hoverElement.remove();
        },
        show: function() {
            var view = this;
        ...
        showItems: function() {
            ...
            var n_slides = this.$(".swiper-slide").length, n_actual_slides = n_slides / 3;
        ...
        hideItems: function() {
            var view = this, delay = 0, timeout = 0, n_slides = this.$(".swiper-slide").length, n_actual_slides = n_slides / 3;
...
var VSLV_APP = function(page_module, project_module, discovery_module, app_data) {
    var Router = Backbone.Router.extend({
        ...
        project: function(slug, mediaIndex) {
            var router = this, project_model = project_module.collection.findWhere({
                slug: slug
            });
            ...
            if (page_module.currentPageView.model.get("slug") === "portfolio") {
                this.listenToOnce(project_module.portfolioView, "PortfolioView:is-closed", function() {
        ...
        page: function(slug) {
            ...
            page_module.currentPageView.listenToOnce(page_module.currentPageView, "PageView:is-hidden", function() {
                ...
                $("body").addClass(slug);
                if (slug === "portfolio") {
                    page_module.currentPageView.$el.css({
            ...
            switch (slug) {
              case "portfolio":
                ...
                if (project_module.portfolioView.project_thumbs_loaded) {
                    ...
                    project_module.portfolioView.listenToOnce(project_module, "PortfolioView:thumbs-loaded", project_module.portfolioView.show);
                }
                break;
        ...
        updateLanguageSwitcher: function(slug) {
            var model = page_module.collection.findWhere({
                slug: slug
            }) || project_module.collection.findWhere({
                slug: slug
            });
            if (!model) {
                return;
            }
            var $ls = $("body > header .language-switcher"), $ls_links = $ls.find("a");
            _.each(model.get("languages"), function(value, key, collection) {
                var $link = $ls.find("." + key + " a");
    ...
    return {
        ...
        common: {
            init: function() {
                ...
                page_module.on("Pages:loaded", function() {}, this);
                project_module.on("Projects:loaded", function() {}, this);
                currentPage = page_module.init(app_data.currentPost);
                project_module.init(app_data.projects);
                $.when(page_module.collection_fetch_promise, project_module.collection_fetch_promise).then($.proxy(function(pages, projects) {
                    console.log(pages);
                    console.log(projects);
                    $("main > .preloader").transition({
                        opacity: 0
                    }).removeClass("animate");
                    ...
                    if (currentPage.get("slug") === "portfolio") {
                        project_module.portfolioView.listenTo(project_module, "PortfolioView:thumbs-loaded", function() {
                            project_module.portfolioView.show();
                ...
                discovery_module.discoveryView.on("DiscoveryView:setAndRenderEnded", function(discoveryModel) {
                    ...
                    if (discoveryModel.get("type") === "project") {
                        setTimeout(function() {}, VSLV_CONFIG.discovery_hide_content_delay);
                ...
                project_module.portfolioView.on("PortfolioView:items-shown", function() {
                    if (!this.portoflio_position_randomized) {
                        ...
                        this.swiper.setWrapperTransition(1e3);
            ...
            initMainMenu: function() {
                ...
                function changeLogo(type) {
                    var logo_src = $(".navbar .navbar-brand img").attr("src"), new_logo_filename = "logo-vslv-" + type + ".svg", new_logo_src = logo_src.replace(/[^\/?]*\.[^\/?]*(\?.*)?$/, new_logo_filename);
            ...
            ie_vslv_logo_height_fix: function() {
                var $img = $(".navbar-brand img"), img = $img.get(0), w = 290, h = 34, ratio = h / w, nh = $img.width() * ratio;
